...
func TestSql(t *testing.T) {
	dialect := testDialect{}
	sql, args, err := Sql(`SELECT count(1) AS "x"`).StringWithArgs(dialect, []any{})
	expectedArgs := []any{}
	...
	sql, args, err = Sql("SELECT * FROM x WHERE y = ", Param(100), " AND z IS ", Param(true)).StringWithArgs(dialect, []any{})
	expectedArgs = []any{100, true}
