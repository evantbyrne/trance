...
func TestColumnType(t *testing.T) {
	...
	type testModel struct {
		...
		Custom         []byte                           `@:"test_custom" @type:"JSONB NOT NULL"`
		Default        string                           `@:"test_default" @default:"'foo'" @length:"100"`
		Float          float64                          `@:"test_float"`
		FloatNull      sql.NullFloat64                  `@:"test_float_null"`
		Id             int64                            `@:"test_id" @primary:"true"`
		Int            int32                            `@:"test_int"`
		...
		TimeZone       time.Time                        `@:"test_time_zone" @time_zone:"true"`
		Varchar        string                           `@:"test_varchar" @length:"100"`
		VarcharNull    sql.NullString                   `@:"test_varchar_null" @length:"50"`
		ForiegnKey     trance.ForeignKey[testFkString]  `@:"test_fk_id" @on_delete:"CASCADE" @on_update:"CASCADE"`
		ForiegnKeyNull trance.NullForeignKey[testFkInt] `@:"test_fk_null_id" @on_delete:"SET NULL"`
