...
func TestQueryFilters(t *testing.T) {
	...
	PurgeWeaves()
	query := Query[testModel]().Filter("a", "=", "foo")
...
func TestQueryJoins(t *testing.T) {
	...
	query.Join("testgroups", Q("testgroups.id", "=", "testaccounts.group_id"))
	expected := []JoinClause{
		{
			Direction: "INNER",
			Table:     "testgroups",
	...
	query = Query[testAccounts]().JoinLeft("testgroups", Q("testgroups.id", "=", "testaccounts.group_id"))
	expected[0].Direction = "LEFT"
	assertJoins(t, expected, query.Config.Joins)

	query = Query[testAccounts]().JoinRight("testgroups", Q("testgroups.id", "=", "testaccounts.group_id"))
	expected[0].Direction = "RIGHT"
	assertJoins(t, expected, query.Config.Joins)

	query = Query[testAccounts]().JoinFull("testgroups", Or(
...
type testGroupsQuerySlice struct {
	Accounts OneToMany[testAccountsQuerySlice] `@:"group_id"`
	Id       int64                             `@:"id" @primary:"true"`
	Name     string                            `@:"name" @length:"100"`
}
type testAccountsQuerySlice struct {
	Group NullForeignKey[testGroupsQuerySlice] `@:"group_id" @on_delete:"SET NULL"`
	Id    int64                                `@:"id" @primary:"true"`
	Name  string                               `@:"name"`
}
...
func TestQuerySlice(t *testing.T) {
	...
	PurgeWeaves()
	query := Query[testAccountsQuerySlice]()
	...
	// One to many.
	PurgeWeaves()
	query2 := Query[testGroupsQuerySlice]()
	...
	actual2, err := query2.slice()
	if err != nil {
		t.Fatal("Unexpected error:", err)
	}
	expected2 := make([][]map[string]any, 0)
	expected2 = append(expected2, []map[string]any{
		{
			"Id":      int64(1),
			"Name":    "foo",
			"GroupId": int64(10),
